---
title: "Meta Analysis Final Product V5"
author: "Matt Dietz"
date: "2/22/2022"
...
fix contradiciton length to be robust to more than just "no" = 1
...
```{r setup}
# knitr::opts_knit$set(root.dir = normalizePath("C:\\Users\\rgast\\OneDrive\\Downloads\\"))
```
...
new_report_path <- "C:\\Users\\rgast\\OneDrive\\Documents\\March 22 report.pdf"
reports_files <- c("KAVES report.pdf", "HIFIVE report.pdf", new_report_path) 
...
for (i in 1:length(reports_files))  {      #load each file into R separately 
  ...
  if (grepl("\\", reports_files[i], fixed = TRUE)){
   name <- strsplit(name, "\\", fixed = T)[[1]]
   name <- tail(name, n=1)
  }
...
create_chapters <- function(reports) {
  ...
  chap_findings <- reports[table_of_contents[finding_pg,2]:table_of_contents[next_num(finding_pg,which(grepl("conclusion", table_of_contents[,1]))),2]]
  ...
  chap_findings[length(chap_findings)] <- as.character(str_split(chap_findings[[length(chap_findings)]],removeNumbers(toupper(table_of_contents[next_num(finding_pg,which(grepl("conclusion",table_of_contents[,1]))),1])))[[1]][1])
  ...
  annex_pg <- next_num(recommend_pg,which(grepl("annex|appendix", table_of_contents[,1])))
...
text_rating <- function(text,sent_bag,contras) {
  ...
  if(length(text) != 0) {
    for (i in 1:length(text)) {
      ...
      # if (gut_reactions[i]==0 & counter==1){
      #   gut_reactions[i] <- -2
      # }
...
grader <- function(scores, eval_questions)  {
  ...
  for (i in 1:length(scores)) {   #first get the score for each findings chapter by eval question
    ...
    chap_score[3] <- mean(chap_word_score)
  ...
  colnames(word_score) <- c("Raw Score","Weighted Average Score","Word's Average Score","Word's Simple Average Score")
  rownames(chapter_score) <- names(scores)
  names(chapter_score) <- c("Raw Score","Weighted Average Score", "Simple Average")
...
for (i in 1:length(reports[[3]])) {
  ...
  final_acr <- which(grepl("EXECUTIVE SUMMARY",reports[[i]]))
  first_acr <- tail(first_acr, n=1)
  final_acr <- tail(final_acr, n=1)
  print(first_acr)
  print(final_acr)
...
for(report in 1:length(reports)){
  ...
  for (chapter in 1:length(all_chapters)){
    ...
    section_name <- c(section_name, all_chapters[[chapter]])
  ...
  author_baseline_df <- data.frame( net_score, count_pos, count_neg, section_rating)

  totals_auth_baseline <- c(sum(net_score),sum(count_pos),sum(count_neg), sum(net_score)/(sum(count_pos) +sum(count_neg)))
...
chapters_conclusion <- list()
for (i in 1:length(reports)) {
  chapters_conclusion[[paste("report",i)]] <- chapters[[i]]$conclusions
}

key_sentences_ <- list() ### this is not the final key_sentences
for (i in 1:length(chapters_conclusion)) {
  name <- paste("Report", i)
  key_sentences_[[name]] <- sentence_extraction_(chapters_conclusion[[i]],selected_key_words[[i]],2)
}
...
# for (i in 1:length(chapters_conclusion)) {
#   name <- paste("Report", i)
#   key_sentences[[name]] <- sentence_extraction(chapters_conclusion[[i]],selected_key_words[[i]],2, all_tables[[i]])
# }
...
#Get the sentiment score of the findings section for each key word and then score the key words overall. 
key_grades_conc <- list()
report_grades_conc <- list()

for (i in 1:length(reports)) {
  key_grades_conc[[paste("report",i)]] <- find_sentiments(list(key_sentences_[[i]]),afinn_edited,contradictions)[[1]]
  report_grades_conc[[paste("report",i)]] <- grader(list(key_grades[[i]]))
}
